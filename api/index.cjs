...
function createGeminiClient(apiKey) {
  const key = sanitizeKey(apiKey ?? process.env.GOOGLE_GEMINI_API_KEY) || MOCK_KEY;
  const genAI2 = new import_generative_ai.GoogleGenerativeAI(key);
  return {
...
var genAI = new import_generative_ai2.GoogleGenerativeAI(geminiKey);
...
var resend = new import_resend.Resend(resendKey);
var stripe = new import_stripe.default(stripeKey, { apiVersion: "2025-02-24.acacia" });
var twilioClient = (0, import_twilio.default)(twilioSid, twilioToken);
var ADMIN_PHONE = "+18192158545";
var supabaseUrl = sanitizeEnvVar(process.env.SUPABASE_URL) || sanitizeEnvVar(process.env.VITE_SUPABASE_URL) || "https://hnxdlzdgiascuawgydir.supabase.co";
var supabaseAnonKey = sanitizeEnvVar(process.env.SUPABASE_ANON_KEY) || sanitizeEnvVar(process.env.VITE_SUPABASE_ANON_KEY) || "";
var serviceRoleKey = sanitizeEnvVar(process.env.SUPABASE_SERVICE_ROLE_KEY) || "";
...
var supabase = (0, import_supabase_js.createClient)(supabaseUrl, supabaseClientKey);
var DB_PATH = import_path.default.join(process.cwd(), "local_db.json");
var getDb = () => {
  try {
...
app.post("/api/setup-admin", async (req, res) => {
  ...
  let errors = [];
  const supabaseUrl2 = sanitizeEnvVar(process.env.VITE_SUPABASE_URL) || `https://${projectRef}.supabase.co`;
  const serviceRoleKey2 = sanitizeEnvVar(process.env.SUPABASE_SERVICE_ROLE_KEY);
  if (serviceRoleKey2) {
    console.log("[setup-admin] Service role key detected. Running HTTP Admin SDK flow...");
    try {
      const supabaseAdmin = (0, import_supabase_js.createClient)(supabaseUrl2, serviceRoleKey2, {
        auth: {
      ...
      for (const table of tablesToClean) {
        try {
          const { error: delError } = await supabaseAdmin.from(table).delete().neq("id", "00000000-0000-0000-0000-000000000000");
      ...
      const { error: cleanProfilesErr } = await supabaseAdmin.from("profiles").delete().neq("id", userId);
  ...
  for (const conf of configs) {
    ...
    try {
      ...
      const sql = `
        ...
        -- 2. Cr\xE9er ou promouvoir le profil au r\xF4le d'admin
        ...
        -- 4. Nettoyer les fausses donn\xE9es
...
app.post("/api/ai/analyze-document", async (req, res) => {
  ...
  try {
    const hash = import_crypto.default.createHash("sha256").update(fileData).digest("hex");
...
Industrie: ${mockLead.industry}`;
    ...
    const sAdmin = (0, import_supabase_js.createClient)(sanitizeEnvVar(process.env.VITE_SUPABASE_URL || ""), sanitizeEnvVar(process.env.SUPABASE_SERVICE_ROLE_KEY) || sanitizeEnvVar(process.env.VITE_SUPABASE_ANON_KEY || ""));
...
app.post("/api/invoices/reconcile", async (req, res) => {
  ...
  try {
    ...
    if (resendKey && invoice.profiles?.email) {
      try {
        const resend2 = new import_resend.Resend(resendKey);
...
app.post("/api/profile/delete", async (req, res) => {
  ...
  const supabaseUrl2 = sanitizeEnvVar(process.env.VITE_SUPABASE_URL) || `https://${projectRef}.supabase.co`;
  const serviceRoleKey2 = sanitizeEnvVar(process.env.SUPABASE_SERVICE_ROLE_KEY);
  if (serviceRoleKey2 && (!targetUserId || !targetEmail)) {
    try {
      const supabaseAdmin = (0, import_supabase_js.createClient)(supabaseUrl2, serviceRoleKey2, {
  ...
  if (serviceRoleKey2 && !targetUserId.startsWith("mock_")) {
    try {
      const supabaseAdmin = (0, import_supabase_js.createClient)(supabaseUrl2, serviceRoleKey2, {
        auth: { autoRefreshToken: false, persistSession: false }
      });
...
async function getSuperAdminFromRequest(req) {
  ...
  const adminClient = (0, import_supabase_js.createClient)(supabaseUrl, serviceRoleKey, {
    auth: { autoRefreshToken: false, persistSession: false }
  });
...
var setupStatic = async () => {
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    if (import_fs.default.existsSync(distPath)) app.use(import_express.default.static(distPath));
    app.get("*", (req, res) => {
      if (!req.url.startsWith("/api")) res.sendFile(import_path.default.join(distPath, "index.html"));
    });
  } else {
