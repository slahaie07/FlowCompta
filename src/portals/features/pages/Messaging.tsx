...
export function Messaging({ userData, targetClientId }: MessagingProps) {
      ...
      <div className="flex flex-col md:flex-row gap-8 h-[calc(100vh-10rem)]">
        ...
        <Card className="w-full md:w-80 p-0 overflow-hidden flex flex-col shrink-0 border-white/5" glow="gold">
          <div className="px-6 py-5 border-b border-white/5 bg-white/5 shrink-0 flex items-center justify-between">
             <div>
                <h3 className="font-serif font-bold text-ivoire text-base">{t('messaging.adminChannels')}</h3>
                <p className="text-xs text-slate-500 uppercase tracking-widest mt-1 font-black">{t('messaging.activeConversations')}</p>
             </div>
             <Badge variant="gold" className="text-xs py-1 px-2.5 font-black">{clients.length} {t('messaging.activeCount')}</Badge>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar bg-black/20">
            {clients.map(client => (
              ...
              >
                ...
                <p className="text-xs text-slate-500 truncate mt-1.5 font-bold uppercase tracking-tighter">{client.companyName || t('messaging.individual')}</p>
              </button>
            ))}
          </div>
        </Card>
          ...
          {selectedClientId ? (
            <>
              <div className="px-8 py-6 border-b border-white/5 bg-white/5 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-serif font-medium text-ivoire text-lg tracking-tight italic">
                      {t('messaging.filePrefix')} {selectedClient?.displayName}
                    </h3>
                    <div className="flex items-center gap-2 mt-0.5">
                       <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse shadow-[0_0_5px_rgba(34,197,94,0.5)]"></span>
                       <span className="text-xs text-slate-500 uppercase font-bold tracking-widest">{t('messaging.secureChannel')}</span>
                    </div>
                  </div>
                </div>
                <Badge variant="info" className="gap-1.5 py-1 px-3"><Lock size={12}/> {t('messaging.encrypted')}</Badge>
              </div>

              <div className="flex-1 overflow-y-auto p-8 space-y-8 flex flex-col-reverse custom-scrollbar bg-noir/30">
                {loading && <div className="text-center text-slate-500 animate-pulse text-sm">{t('messaging.syncing')}</div>}
              ...
              <div className="p-6 border-t border-white/5 bg-obsidian/50 shrink-0">
                <div className="flex items-center gap-4">
                  <input 
                    ...
                    placeholder={t('messaging.adminPlaceholder')}
                    aria-label={t('messaging.adminPlaceholder')}
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-12 space-y-6">
              ...
              <div className="space-y-2">
                <p className="text-ivoire font-serif text-2xl italic">{t('messaging.noSelection')}</p>
                <p className="text-xs text-slate-500 font-black uppercase tracking-widest max-w-xs mx-auto leading-relaxed">{t('messaging.selectClient')}</p>
              </div>
            </div>
          )}
        </Card>
  ...
  return (
    <Card className="flex flex-col h-[calc(100vh-10rem)] p-0 relative overflow-hidden border-white/5" glow="gold">
      <div className="px-8 py-6 border-b border-white/5 bg-white/5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-serif font-medium text-ivoire text-lg tracking-tight italic">
              {t('messaging.clientTitle')}
            </h3>
            <div className="flex items-center gap-2 mt-0.5">
               <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse shadow-[0_0_5px_rgba(34,197,94,0.5)]"></span>
               <span className="text-xs text-slate-500 uppercase font-bold tracking-widest">{t('messaging.online')}</span>
            </div>
          </div>
        </div>
        <Badge variant="info" className="gap-1.5 py-1 px-3"><Lock size={12}/> {t('messaging.encrypted')}</Badge>
      ...
      <div className="p-6 border-t border-white/5 bg-obsidian/50 shrink-0">
        <div className="flex items-center gap-4">
          <input 
            ...
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && handleSend()}
            placeholder={t('messaging.clientPlaceholder')}
            aria-label={t('messaging.clientPlaceholder')}
            className="flex-1 bg-white/5 border border-white/5 rounded-2xl px-6 py-4 text-sm outline-none text-silver placeholder:text-slate-600 focus:ring-1 focus:ring-gold/30 transition-all shadow-inner"
            disabled={loading}
          />
          <Button 
