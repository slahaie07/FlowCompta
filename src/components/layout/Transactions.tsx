  ...
  return (
    <div className="space-y-10 pb-20 md:pb-10">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
          <h1 className="text-5xl font-serif font-bold text-ivoire tracking-tight leading-tight">Journal <span className="animated-gradient-text italic">{currentMode === 'business' ? 'Flux Pro' : 'Flux Privé'}</span></h1>
          <div className="flex items-center gap-3 mt-3">
            <span className="w-8 h-[1px] bg-gold/50"></span>
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em]">Rapprochement & Conformité temps réel</p>
          </div>
        </div>
        <Button variant="gold" className="gap-3 h-16 px-10 shadow-glow font-black uppercase tracking-[0.2em] text-xs rounded-2xl group" onClick={() => setIsAdding(true)}>
          <Plus size={20} className="group-hover:rotate-90 transition-transform duration-500"/> Saisie Stratégique
        </Button>
      </header>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
         <Card className="p-8 flex items-center gap-8 group hover:border-gold/30 premium-border-gold transition-all duration-700" glow="gold">
            ...
            <div>
               <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-2">Pulsation {currentMode === 'business' ? 'Entrante' : 'Revenus'}</p>
         ...
         <Card className="p-8 flex items-center gap-8 group hover:border-ivoire/20 glass-card transition-all duration-700">
            ...
            <div>
               <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-2">Flux {currentMode === 'business' ? 'Sortant' : 'Dépenses'}</p>
         ...
         <Card className="p-8 flex flex-col justify-center gap-6 md:col-span-2 lg:col-span-1 glass-card">
            <div className="flex items-center justify-between">
               <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Filtre Intelligent</span>
            ...
            <div className="flex gap-3 w-full">
               {['all', 'sale', 'purchase'].map((f) => (
                 ...
                 >
                   {f === 'all' ? 'Universal' : f === 'sale' ? 'Entrants' : 'Sortants'}
                 </button>
               ))}
            </div>
         </Card>
